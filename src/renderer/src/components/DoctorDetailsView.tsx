...
const DoctorDetailsView: React.FC<DoctorDetailsViewProps> = ({ doctor, onBack }) => {
  ...
  const totalCostActive = activeDoctorOrders.reduce(
    (sum: number, order: Order) => sum + order.cost,
    0,
  );
  const totalPaidActive = activeDoctorOrders.reduce(
    (sum: number, order: Order) =>
      sum + order.payments.reduce((pSum: number, p: Payment) => pSum + p.amount, 0),
    0,
  );
  ...
  return (
    <div className="rounded-lg bg-white p-8 shadow-xl">
      ...
      <h2 className="mb-6 flex items-center text-3xl font-bold text-gray-800">
        <User className="mr-3 text-blue-600" size={30} /> Ficha del Doctor: {doctor.title}{' '}
        {doctor.name}
      </h2>
      ...
      <div className="mb-8 grid grid-cols-1 gap-6 md:grid-cols-2">
        <div className="rounded-lg bg-gray-50 p-6 shadow-inner">
          <h3 className="mb-4 flex items-center text-xl font-semibold text-gray-700">
            <Briefcase className="mr-2" /> Información de Contacto
          </h3>
          <p className="mb-2">
            <span className="font-medium text-gray-700">Email:</span> {doctor.email || 'N/A'}
          </p>
          <p className="mb-2">
            <span className="font-medium text-gray-700">Teléfono:</span> {doctor.phone || 'N/A'}
          </p>
          <p className="mb-2">
            <span className="font-medium text-gray-700">Dirección:</span> {doctor.address || 'N/A'}
          </p>
        </div>

        <div className="rounded-lg bg-gray-50 p-6 shadow-inner">
          <h3 className="mb-4 flex items-center text-xl font-semibold text-gray-700">
            <DollarSign className="mr-2" /> Resumen Financiero de Órdenes Activas
          </h3>
          <p className="mb-2 text-lg">
            <span className="font-medium text-gray-700">Costo Total Órdenes Activas:</span>{' '}
            <span className="font-bold text-blue-600">${totalCostActive.toFixed(2)}</span>
          </p>
          <p className="mb-2 text-lg">
            <span className="font-medium text-gray-700">Monto Abonado Órdenes Activas:</span>{' '}
            <span className="font-bold text-green-600">${totalPaidActive.toFixed(2)}</span>
          </p>
          <p className="mb-2 text-lg">
            <span className="font-medium text-gray-700">Saldo Pendiente Órdenes Activas:</span>{' '}
            <span
              className={`font-bold ${totalBalanceActive > 0 ? 'text-red-600' : 'text-green-600'}`}
            >
              ${totalBalanceActive.toFixed(2)}
            </span>
          </p>
        </div>
      </div>
      ...
      <div className="rounded-lg bg-white p-6 shadow-xl">
        <h3 className="mb-4 flex items-center text-xl font-semibold text-gray-800">
          <ClipboardList className="mr-2" /> Casos Activos ({activeDoctorOrders.length})
        </h3>
