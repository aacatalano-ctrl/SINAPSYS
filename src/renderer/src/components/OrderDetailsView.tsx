...
const OrderDetailsView: React.FC<OrderDetailsViewProps> = ({
  ...
  const handleAbonoSubmit = () => {
    ...
    if (amount > pendingBalance) {
      showNotification(
        `El monto no puede ser mayor al saldo pendiente de ${pendingBalance.toFixed(2)}.`,
        'error',
      );
      return;
    }
  ...
  return (
    <div className="rounded-lg bg-white p-8 shadow-xl">
      ...
      <h2 className="mb-6 flex items-center text-3xl font-bold text-gray-800">
        <ClipboardList className="mr-3 text-blue-600" size={30} /> Detalles de la Orden:{' '}
        {order.orderNumber}
      </h2>
      ...
      <div className="mb-8 grid grid-cols-1 gap-6 md:grid-cols-2">
        <div className="rounded-lg bg-gray-50 p-6 shadow-inner">
          <h3 className="mb-4 flex items-center text-xl font-semibold text-gray-700">
            <User className="mr-2" /> Información del Paciente y Doctor
          </h3>
          <p className="mb-2">
            <span className="font-medium text-gray-700">Paciente:</span> {order.patientName}
          </p>
          <p className="mb-2">
        ...
        <div className="rounded-lg bg-gray-50 p-6 shadow-inner">
          <h3 className="mb-4 flex items-center text-xl font-semibold text-gray-700">
            <Calendar className="mr-2" /> Fechas y Estado
          </h3>
          <p className="mb-2">
        ...
        <div className="rounded-lg bg-gray-50 p-6 shadow-inner">
          <h3 className="mb-4 flex items-center text-xl font-semibold text-gray-700">
            <DollarSign className="mr-2" /> Información Financiera
          </h3>
          <p className="mb-2 text-lg">
            <span className="font-medium text-gray-700">Costo Total:</span>{' '}
            <span className="font-bold text-blue-600">${order.cost.toFixed(2)}</span>
          </p>
              ...
              {isAbonando && (
                <div className="mt-4">
                  ...
                  <div className="flex items-center space-x-2">
                    ...
                    <button
                      onClick={() => setIsAbonando(false)}
                      className="rounded-lg bg-gray-300 px-4 py-2 font-bold text-gray-800 hover:bg-gray-400"
                    >
                      Cancelar
                    </button>
                  </div>
      ...
      <div className="rounded-lg bg-white p-6 shadow-xl">
        <h3 className="mb-4 flex items-center text-xl font-semibold text-gray-800">
          <MessageSquare className="mr-2" /> Notas Internas
        </h3>
            ...
            {order.notes.map((note) => (
              ...
              >
                ...
                <p className="mt-2 text-right text-xs text-gray-500">
                  <span className="font-semibold">{note.author}</span> el{' '}
