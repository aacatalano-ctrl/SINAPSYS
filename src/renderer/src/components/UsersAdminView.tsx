...
const UsersAdminView: React.FC<UsersAdminViewProps> = ({ authFetch }) => {
  ...
  const handleUpdateUserStatus = async (userToUpdate: User) => {
    // Changed parameter to User object
    if (userToUpdate.role === 'admin') {
      const masterCode = window.prompt(
        'Este es un usuario administrador. Por favor, introduce el código maestro para continuar:',
      );
    ...
    try {
      ...
      const message =
        err instanceof Error ? err.message : 'Error al actualizar estado del usuario.';
