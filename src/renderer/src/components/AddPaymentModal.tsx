...
interface AddPaymentModalProps {
    ...
    onUpdatePayment: (orderId: string, paymentId: string, paymentData: Partial<Payment>) => Promise<void>;
    paymentToEdit?: Payment | null;
}

const AddPaymentModal: React.FC<AddPaymentModalProps> = ({ order, onClose, isOpen, onAddPayment, onUpdatePayment, paymentToEdit }) => {
  ...
  const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    if (parsedAmount > balance) {
      showNotification(`El monto no puede ser mayor al saldo disponible de ${balance.toFixed(2)}.`, 'error');
      return;
    }
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-gray-600/50">
      <div className="w-full max-w-md rounded-lg bg-white p-8 shadow-xl">
        <h2 className="mb-4 text-2xl font-bold text-gray-800">{isEditMode ? 'Editar Abono' : `Registrar Pago para Orden: ${order.orderNumber}`}</h2>
        <div className="mb-4 rounded-lg border border-blue-200 bg-blue-100 p-3">
          ...
          <p className="text-lg text-blue-900"><span className="font-semibold">Saldo Disponible:</span> ${balance.toFixed(2)}</p>
        </div>
        <form onSubmit={handleSubmit}>
          <div className="mb-4">
            <label htmlFor="paymentAmount" className="mb-2 block text-sm font-semibold text-gray-700">Monto del Abono:</label>
